...
public interface MX11 extends Library {
    ...
    class XID extends NativeLong {
        ...
        protected boolean isNone(final Object o) {
            return o == null || (o instanceof Number && ((Number) o).longValue() == MX11.None);
        }
    ...
    class WindowByReference extends ByReference {
        ...
        public Window getValue() {
            final NativeLong value = getPointer().getNativeLong(0);
            return value.longValue() == MX11.None ? Window.None : new Window(value.longValue());
        }
    }
    ...
    interface Xrender extends Library {
        ...
        class XRenderDirectFormat extends Structure {
            ...
            protected List getFieldOrder() {
                return Arrays.asList(new String[] { "red", "redMask", "green", "greenMask", "blue", "blueMask",
                        "alpha", "alphaMask" });
            }
        }
    ...
    MX11 INSTANCE = (MX11) Native.loadLibrary("X11", MX11.class);

    /*
     * typedef struct { long flags; // marks which fields in this structure are defined Bool input; // does this
     * application rely on the window manager to // get keyboard input? int initial_state; // see below Pixmap
     * icon_pixmap; // pixmap to be used as icon Window icon_window; // window to be used as icon int icon_x, icon_y; //
     * initial position of icon Pixmap icon_mask; // icon mask bitmap XID window_group; // id of related window group //
    ...
    class XWMHints extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "flags", "input", "initial_state", "icon_pixmap", "icon_window",
                    "icon_x", "icon_y", "icon_mask", "window_group" });
        }
    }

    /*
     * typedef struct { unsigned char *value; // same as Property routines Atom encoding; // prop type int format; //
     * prop data format: 8, 16, or 32 unsigned long nitems; // number of data items in value } XTextProperty;
     */
    ...
    /*
     * typedef struct { long flags; // marks which fields in this structure are defined int x, y; // obsolete for new
     * window mgrs, but clients int width, height; /// should set so old wm's don't mess up int min_width, min_height;
     * int max_width, max_height; int width_inc, height_inc; struct { int x; // numerator int y; // denominator }
     * min_aspect, max_aspect; int base_width, base_height; // added by ICCCM version 1 int win_gravity; // added by
     * ICCCM version 1 } XSizeHints;
     */
    class XSizeHints extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "flags", "x", "y", "width", "height", "min_width", "min_height",
                    "max_width", "max_height", "width_inc", "height_inc", "min_aspect", "max_aspect", "base_width",
                    "base_height", "win_gravity" });
        }
    }

    /*
     * typedef struct { int x, y; // location of window int width, height; // width and height of window int
     * border_width; // border width of window int depth; // depth of window Visual *visual; // the associated visual
     * structure Window root; // root of screen containing window #if defined(__cplusplus) || defined(c_plusplus) int
     * c_class; // C++ InputOutput, InputOnly #else int class; // InputOutput, InputOnly #endif int bit_gravity; // one
     * of bit gravity values int win_gravity; // one of the window gravity values int backing_store; // NotUseful,
     * WhenMapped, Always unsigned long backing_planes;// planes to be preserved if possible unsigned long
     * backing_pixel;// value to be used when restoring planes Bool save_under; // boolean, should bits under be saved?
     * Colormap colormap; // color map to be associated with window Bool map_installed; // boolean, is color map
     * currently installed int map_state; // IsUnmapped, IsUnviewable, IsViewable long all_event_masks; // set of events
     * all people have interest in long your_event_mask; // my event mask long do_not_propagate_mask; // set of events
     * that should not propagate Bool override_redirect; // boolean value for override-redirect Screen *screen; // back
     * pointer to correct screen } XWindowAttributes;
     */
    class XWindowAttributes extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "x", "y", "width", "height", "border_width", "depth", "visual", "root",
                    "c_class", "bit_gravity", "win_gravity", "backing_store", "backing_planes", "backing_pixel",
                    "save_under", "colormap", "map_installed", "map_state", "all_event_masks", "your_event_mask",
                    "do_not_propagate_mask", "override_redirect", "screen" });
        }
    }

    /*
     * typedef struct { Pixmap background_pixmap; // background or None or ParentRelative unsigned long
     * background_pixel; // background pixel Pixmap border_pixmap; // border of the window unsigned long border_pixel;
     * // border pixel value int bit_gravity; // one of bit gravity values int win_gravity; // one of the window gravity
     * values int backing_store; // NotUseful, WhenMapped, Always unsigned long backing_planes;// planes to be preseved
     * if possible unsigned long backing_pixel;// value to use in restoring planes Bool save_under; // should bits under
     * be saved? (popups) long event_mask; // set of events that should be saved long do_not_propagate_mask; // set of
     * events that should not propagate Bool override_redirect; // boolean value for override-redirect Colormap
     * colormap; // color map to be associated with window Cursor cursor; // cursor to be displayed (or None) }
    ...
    class XSetWindowAttributes extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "background_pixmap", "background_pixel", "border_pixmap",
                    "border_pixel", "bit_gravity", "win_gravity", "backing_store", "backing_planes", "backing_pixel",
                    "save_under", "event_mask", "do_not_propagate_mask", "override_redirect", "colormap", "cursor" });
        }
    }
    ...
    class XVisualInfo extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "visual", "visualid", "screen", "depth", "c_class", "red_mask",
    ...
    void XSetWMProperties(Display display, Window window, String window_name, String icon_name, String[] argv,
            int argc, XSizeHints normal_hints, Pointer wm_hints, Pointer class_hints);
    ...
    /**
     * Flushes the output buffer. Most client applications need not use this function because the output buffer is
     * automatically flushed as needed by calls to XPending, XNextEvent, and XWindowEvent. Events generated by the
     * server may be enqueued into the library's event queue.
     */
    int XFlush(Display display);

    /**
     * Flushes the output buffer and then waits until all requests have been received and processed by the X server. Any
     * errors generated must be handled by the error handler. For each protocol error received by Xlib, XSync calls the
     * client application's error handling routine (see section 11.8.2). Any events generated by the server are enqueued
     * into the library's event queue.<br/>
     * Finally, if you passed False, XSync does not discard the events in the queue. If you passed True, XSync discards
     * all events in the queue, including those events that were on the queue before XSync was called. Client
     ...
     * performs a system call). If mode is QueuedAfterFlush, XEventsQueued returns the number of events already in the
     * queue if the number is nonzero. If there are no events in the queue, XEventsQueued flushes the output buffer,
     * attempts to read more events out of the application's connection, and returns the number read. If mode is
     * QueuedAfterReading, XEventsQueued returns the number of events already in the queue if the number is nonzero. If
     * there are no events in the queue, XEventsQueued attempts to read more events out of the application's connection
     * without flushing the output buffer and returns the number read.<br/>
     * XEventsQueued always returns immediately without I/O if there are events already in the queue. XEventsQueued with
     * mode QueuedAfterFlush is identical in behavior to XPending. XEventsQueued with mode QueuedAlready is identical to
     * the XQLength function.
     */
    int XEventsQueued(Display display, int mode);

    /**
     * Returns the number of events that have been received from the X server but have not been removed from the event
    ...
    class XGCValues extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "function", "plane_mask", "foreground", "background", "line_width",
                    "line_style", "cap_style", "join_style", "fill_style", "fill_rule", "arc_mode", "tile", "stipple",
                    "ts_x_origin", "ts_y_origin", "font", "subwindow_mode", "graphics_exposures", "clip_x_origin",
                    "clip_y_origin", "clip_mask", "dash_offset", "dashes" });
        }
    }
    ...
    boolean XQueryPointer(Display display, Window window, WindowByReference root_return,
            WindowByReference child_return, IntByReference root_x_return, IntByReference root_y_return,
            IntByReference win_x_return, IntByReference win_y_return, IntByReference mask_return);
    ...
    int CopyFromParent = 0; /*
                             * border pixmap in CreateWindow and ChangeWindowAttributes special VisualID and special
                             * window class passed to CreateWindow
                             */
    ...
    /*
     * Input Event Masks. Used as event-mask window attribute and as arguments to Grab requests. Not to be confused with
     * event names.
     */
    int NoEventMask = 0;
    ...
    /*
     * Event names. Used in "type" field in XEvent structures. Not to be confused with event masks above. They start
     * from 2 because 0 and 1 are reserved in the protocol for errors and replies.
     */
    int KeyPress = 2;
    int KeyRelease = 3;
    int ButtonPress = 4;
    ...
    /*
     * Key masks. Used as modifiers to GrabButton and GrabKey, results of QueryPointer, state in various key-, mouse-,
     * and button-related events.
     */
    int ShiftMask = (1 << 0);
    int LockMask = (1 << 1);
    int ControlMask = (1 << 2);
    ...
    /*
     * modifier names. Used to build a SetModifierMapping request or to read a GetModifierMapping request. These
     * correspond to the masks defined above.
     */
    int ShiftMapIndex = 0;
    int LockMapIndex = 1;
    ...
    /*
     * button names. Used as arguments to GrabButton and as detail in ButtonPress and ButtonRelease events. Not to be
     * confused with button masks above. Note that 0 is already defined above as "AnyButton".
     */
    int Button1 = 1;
    int Button2 = 2;
    int Button3 = 3;
    ...
    int BadAccess = 10; /*
                         * depending on context: - key/button already grabbed - attempt to free an illegal cmap entry -
                         * attempt to store into a read-only color map entry. - attempt to modify the access control
    ...
    /*
     * Display classes used in opening the connection Note that the statically allocated ones are even numbered and the
    ...
    class XKeyEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "root",
                    "subwindow", "time", "x", "y", "x_root", "y_root", "state", "keycode", "same_screen" });
        }
    }
    ...
    class XButtonEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "root",
                    "subwindow", "time", "x", "y", "x_root", "y_root", "state", "button", "same_screen" });
        }
    }
    ...
    public static class XClientMessageEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "message_type",
                    "format", "data" });
        }
    ...
    class XMotionEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "root",
                    "subwindow", "time", "x", "y", "x_root", "y_root", "state", "is_hint", "same_screen" });
        }
    }
    ...
    class XCrossingEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "root",
                    "subwindow", "time", "x", "y", "x_root", "y_root", "mode", "detail", "same_screen", "focus",
                    "state" });
        }
    }
    ...
    class XFocusChangeEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays
                    .asList(new String[] { "type", "serial", "send_event", "display", "window", "mode", "detail" });
        }
    }
    ...
    class XExposeEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "x", "y", "width",
                    "height", "count" });
        }
    }
    ...
    class XGraphicsExposeEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "drawable", "x", "y",
                    "width", "height", "count", "major_code", "minor_code" });
        }
    }
    ...
    class XNoExposeEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "drawable", "major_code",
                    "minor_code" });
        }
    }
    ...
    class XCreateWindowEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "parent", "window", "x",
                    "y", "width", "height", "border_width", "override_redirect" });
        }
    }
    ...
    class XDestroyWindowEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "display", "send_event", "serial", "type", "event", "window" });
        }
    }
    ...
    class XUnmapEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "event",
                    "from_configure" });
        }
    }
    ...
    class XReparentEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "event", "window", "parent",
                    "x", "y", "override_redirect" });
        }
    }
    ...
    class XGravityEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays
                    .asList(new String[] { "type", "serial", "send_event", "display", "event", "window", "x", "y" });
        }
    }
    ...
    class XResizeRequestEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays
                    .asList(new String[] { "type", "serial", "send_event", "display", "window", "width", "height" });
        }
    }
    ...
    class XConfigureRequestEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "parent", "window", "x",
                    "y", "width", "height", "border_width", "above", "detail", "value_mask" });
        }
    }
    ...
    class XCirculateEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays
                    .asList(new String[] { "type", "serial", "send_event", "display", "event", "window", "place" });
        }
    }
    ...
    class XCirculateRequestEvent extends Structure {
        ...
        protected List getFieldOrder() {
            return Arrays
                    .asList(new String[] { "type", "serial", "send_event", "display", "parent", "window", "place" });
        }
    }
    ...
    class XPropertyEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "atom", "time",
                    "state" });
        }
    }
    ...
    class XSelectionClearEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "selection",
                    "time" });
        }
    }
    ...
    class XSelectionRequestEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "owner", "requestor",
    ...
    class XSelectionEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "requestor", "selection",
    ...
    class XColormapEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "colormap",
                    "c_new", "state" });
        }
    }
    ...
    class XMappingEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "serial", "send_event", "display", "window", "request",
                    "first_keycode", "count" });
        }
    }
    ...
    class XErrorEvent extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "type", "display", "serial", "error_code", "request_code",
                    "minor_code", "resourceid" });
        }
    }
    ...
    /**
     * Returns an array of {@link XVisualInfo} which must be freed by {@link #XFree}. Use
     * {@link XVisualInfo#toArray(int) toArray(nitems_return.getValue()} to obtain the array.
     */
    ...
    int XPutImage(Display dpy, Drawable d, GC gc, XImage image, int src_x, int src_y, int dest_x, int dest_y,
     ...
     * @param modifiers Specifies the set of keymasks or {@link #AnyModifier}. The mask is the bitwise inclusive OR of
     *            the valid keymask bits.
    ...
    // num_codes);
    /**
     * Defines the symbols for the specified number of KeyCodes starting with first_keycode. The symbols for KeyCodes
     * outside this range remain unchanged. The number of elements in keysyms must be: num_codes * keysyms_per_keycode.
     * The specified first_keycode must be greater than or equal to min_keycode returned by XDisplayKeycodes, or a
     * BadValue error results. In addition, the following expression must be less than or equal to max_keycode as
     * returned by XDisplayKeycodes, or a BadValue error results: first_keycode + num_codes - 1.
     */
    int XChangeKeyboardMapping(Display display, int first_keycode, int keysyms_per_keycode, KeySym[] keysyms,
            int num_codes);

    /**
     * Returns the symbols for the specified number of KeyCodes starting with first_keycode. The value specified in
     * first_keycode must be greater than or equal to min_keycode as returned by XDisplayKeycodes, or a BadValue error
     * results. In addition, the following expression must be less than or equal to max_keycode as returned by
     * XDisplayKeycodes: first_keycode + keycode_count - 1. If this is not the case, a BadValue error results. The
     * number of elements in the KeySyms list is: keycode_count * keysyms_per_keycode_return. KeySym number N, counting
     * from zero, for KeyCode K has the following index in the list, counting from zero: (K - first_code) *
     * keysyms_per_code_return + N. The X server arbitrarily chooses the keysyms_per_keycode_return value to be large
     * enough to report all requested symbols. A special KeySym value of NoSymbol is used to fill in unused elements for
     * individual KeyCodes. To free the storage returned by XGetKeyboardMapping, use XFree.
     */
    KeySym XGetKeyboardMapping(Display display, byte first_keycode, int keycode_count,
            IntByReference keysyms_per_keycode_return);

    /**
     * Returns the min-keycodes and max-keycodes supported by the specified display. The minimum number of KeyCodes
     * returned is never less than 8, and the maximum number of KeyCodes returned is never greater than 255. Not all
    ...
    /**
     * Specifies the KeyCodes of the keys (if any) that are to be used as modifiers. If it succeeds, the X server
     * generates a MappingNotify event, and XSetModifierMapping returns MappingSuccess. X permits at most 8 modifier
     * keys. If more than 8 are specified in the XModifierKeymap structure, a BadLength error results.
     */
    int XSetModifierMapping(Display display, XModifierKeymapRef modmap);

    /**
     * The XGetModifierMapping function returns a pointer to a newly created XModifierKeymap structure that contains the
     * keys being used as modifiers. The structure should be freed after use by calling XFreeModifiermap. If only zero
    ...
    /**
     * Adds the specified KeyCode to the set that controls the specified modifier and returns the resulting
    ...
    /**
     * Deletes the specified KeyCode from the set that controls the specified modifier and returns a pointer to the
    ...
    /**
     * Rings the bell on the keyboard on the specified display, if possible. The specified volume is relative to the
     * base volume for the keyboard. If the value for the percent argument is not in the range -100 to 100 inclusive, a
     * BadValue error results. The volume at which the bell rings when the percent argument is nonnegative is: base -
     * [(base * percent) / 100] + percent. The volume at which the bell rings when the percent argument is negative is:
     * base + [(base * percent) / 100]. To change the base volume of the bell, use XChangeKeyboardControl.
     */
    int XBell(Display display, int percent);

    /**
     * Returns a bit vector for the logical state of the keyboard, where each bit set to 1 indicates that the
     * corresponding key is currently pressed down. The vector is represented as 32 bytes. Byte N (from 0) contains the
     * bits for keys 8N to 8N + 7 with the least significant bit in the byte representing key 8N. Note that the logical
     * state of a device (as seen by client applications) may lag the physical state if device event processing is
     * frozen.
     */
    int XQueryKeymap(Display display, byte[] keys_return);

    /**
     * The modifiermap member of the XModifierKeymap structure contains 8 sets of max_keypermod KeyCodes, one for each
     * modifier in the order Shift, Lock, Control, Mod1, Mod2, Mod3, Mod4, and Mod5. Only nonzero KeyCodes have meaning
     * in each set, and zero KeyCodes are ignored. In addition, all of the nonzero KeyCodes must be in the range
     * specified by min_keycode and max_keycode in the Display structure, or a BadValue error results.
     */
    ...
    class XKeyboardControlRef extends Structure implements Structure.ByReference {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "key_click_percent", "bell_percent", "bell_pitch", "bell_duration",
    ...
    class XKeyboardStateRef extends Structure implements Structure.ByReference {
        ...
        protected List getFieldOrder() {
            return Arrays.asList(new String[] { "key_click_percent", "bell_percent", "bell_pitch", "bell_duration",
                    "led_mask", "global_auto_repeat", "auto_repeats" });
        }
